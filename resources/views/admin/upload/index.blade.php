...
@section('content')
    ...
    <xblock>
        ...
        {{--<a href="{{url('admin/upload/create')}}" i class="layui-btn layui-icon">添加</a>--}}
    ...
    <table class="layui-table">
        ...
        @foreach($data as $v)
            <tr>
                ...
                {{--</td>--}}
                <td>{{$v->gp_id}}</td>
                <td><img src="{{url("$v->gp_photo")}}" alt=""></td>
                <td><img src="{{url("$v->gp_thum")}}" alt=""></td>
                <td><img src="{{url("$v->gp_small")}}" alt=""></td>
                <td>{{date('Y:m:d H:i:s',$v->gp_createtime)}}</td>
                <td class="td-manage">
                    <a title="删除"  href="javascript:;" onclick="member_del({{$v->gp_id}});">
    ...
    <script>
        ...
        function member_del(obj,id){
            layer.confirm('确认要删除吗？',function(index){
                //发异步删除数据
                $.post("{{url('admin/upload/del')}}/"+obj,{'_token':"{{csrf_token()}}"},function(data){
                    if (data.status ==0){
